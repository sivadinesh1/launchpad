...
export class SalesReturnDialogComponent implements OnInit {
    ...
    processItems(element) {
        this.returnArr.push({
            ...
            after_tax_value: element.after_tax_value,
    ...
    update_change(event$, idx) {
        if (
            this.returnArr[idx].returned + +event$.target.value >
            this.returnArr[idx].qty
        ) {
        ...
        this.returnArr[idx].total_value = +(
            (event$.target.value *
                this.returnArr[idx].mrp *
                (100 - this.returnArr[idx].disc_percent)) /
            100
        ).toFixed(2);
        this.returnArr[idx].tax_value = +(
            this.returnArr[idx].total_value -
            this.returnArr[idx].after_tax_value
        ).toFixed(2);

        if (this.returnArr[idx].igst !== 0.0) {
    ...
    update_disc_value(event$, idx) {
        ...
        this.returnArr[idx].total_value = +(
            (this.returnArr[idx].received_now *
                this.returnArr[idx].mrp *
    ...
    async return() {
        ...
        const isItemsToReturn1 = this.returnArr.some(
            (e) => e.received_now > e.qty + e.returned || e.error !== ''
        );

        if (isItemsToReturn && !isItemsToReturn1) {
            console.log('object/... su bit ');

            this.filteredArr = this.returnArr.filter(
                (element) => element.received_now > 0
            );

            const sum_to_return_amount = this.returnArr
                .reduce(
                    (accumulator, currentValue) =>
                        accumulator + currentValue.total_value,
                    0
                )
                .toFixed(2);
            const sum_to_return_items = this.returnArr.reduce(
                (accumulator, currentValue) =>
                    accumulator + currentValue.received_now,
                0
            );
            ...
            this.nothingToReturnMsg =
                'Nothing to return (or) returns not matching';
        }
    }
