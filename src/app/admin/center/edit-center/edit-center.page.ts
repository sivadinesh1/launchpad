...
export class EditCenterPage implements OnInit {
    ...
    setFormValues() {
        this.submitForm
            .get('formArray')
            .get([0])
            .patchValue({ center_id: this.center_id });
        this.submitForm
            .get('formArray')
            .get([0])
            .patchValue({ company_id: this.resultList.company_id });
        this.submitForm
            .get('formArray')
            .get([0])
            .patchValue({ name: this.resultList.name });
        this.submitForm
            .get('formArray')
            .get([0])
            .patchValue({ address1: this.resultList.address1 });
        this.submitForm
            .get('formArray')
            .get([0])
            .patchValue({ address2: this.resultList.address2 });
        this.submitForm
            .get('formArray')
            .get([0])
            .patchValue({ address3: this.resultList.address3 });
        this.submitForm
            .get('formArray')
            .get([0])
            .patchValue({ district: this.resultList.district });
        this.submitForm
            .get('formArray')
            .get([0])
            .patchValue({ state_id: this.resultList.state_id });
        this.submitForm
            .get('formArray')
            .get([0])
            .patchValue({ pin: this.resultList.pin });

        this.submitForm
            .get('formArray')
            .get([1])
            .patchValue({ gst: this.resultList.gst });
        this.submitForm
            .get('formArray')
            .get([1])
            .patchValue({ phone: this.resultList.phone });
        this.submitForm
            .get('formArray')
            .get([1])
            .patchValue({ mobile: this.resultList.mobile });
        this.submitForm
            .get('formArray')
            .get([1])
            .patchValue({ mobile2: this.resultList.mobile2 });
        this.submitForm
            .get('formArray')
            .get([1])
            .patchValue({ whatsapp: this.resultList.whatsapp });

        this.submitForm
            .get('formArray')
            .get([2])
            .patchValue({ email: this.resultList.email });

        this.submitForm
            .get('formArray')
            .get([2])
            .patchValue({ bankname: this.resultList.bankname });
        this.submitForm
            .get('formArray')
            .get([2])
            .patchValue({ accountno: this.resultList.accountno });
        this.submitForm
            .get('formArray')
            .get([2])
            .patchValue({ ifsccode: this.resultList.ifsccode });
        this.submitForm
            .get('formArray')
            .get([2])
            .patchValue({ branch: this.resultList.branch });
    }
    ...
    submit() {
        this._commonApiService
            .updateCenter(this.submitForm.value)
            .subscribe((data: any) => {
                this.updateSnackBar();
            });
    }
            ...
            Img.onload = (e: any) => {
                ...
                if (position === 'side') {
                    if (width > 380 || height > 101) {
                        this.sideImageError =
                            'Upload after Resizing Logo to 380px X 100px';
                        this.imageError = '';
                        this._cdr.detectChanges();
                        return false;
                    } else {
                    ...
                    if (width > 110 || height > 110) {
                        this.imageError =
                            'Upload after Resizing Logo to 110px X 110px';
                        this.sideImageError = '';
                        this._cdr.detectChanges();
                        return false;
                    } else {
        ...
        this._commonApiService
            ...
            .subscribe((data: any) => {
                ...
                if (response.result === 'ok') {
                    ...
                    if (position === 'side') {
                        this.openSnackBar(
                            'Side Logo Uploaded Successfully',
                            ''
                        );
                    } else {
