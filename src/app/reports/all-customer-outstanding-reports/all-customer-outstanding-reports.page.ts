    ...
    reloadOutstandingBalance() {
        this._commonApiService
            ...
            .subscribe((data: any) => {
                ...
                this.balance = this.outstandingBalanceList.reduce(
                    (total, e) => total + e.balance_amt,
                    0
                );
    ...
    downloadReport() {
        ...
        this.outstandingBalanceList.forEach((element) => {
            if (element.last_paid_date !== null) {
                element.last_paid_date = moment(element.last_paid_date).format(
                    'DD-MM-YYYY'
                );
            }
        });
        ...
        const reportData = JSON.parse(
            JSON.stringify(this.outstandingBalanceList)
        );
